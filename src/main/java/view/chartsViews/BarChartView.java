...
/**
 * Questa classe rappresenta la view corrispondente al grafico {@link javafx.scene.chart.BarChart
 * BarChart}. L'aggiornamento della view è eseguita secondo i canoni del design pattern MVC.
 *
 * @author ingsw20
 */
public class BarChartView implements Observer, ChartInterface {
  ...
  /**
   * Costruttore per l'inizializzazione del grafico relativo ad un singolo evento, inoltre registra
   * la view presso il model.
   *
  ...
  /**
   * il construttore inizializza {@link #barChart} per la gestione del grafico, inoltre registra la
   * view presso il model. In particolare, questo costruttore è utilizzato per la visualizzazione
    ...
    for (int i = 0; i < eventNames.size(); i++) {
      ...
      for (int j = 0; j < datas.size(); j++) {
        if (datas.get(j).getXValue().equals(eventNames.get(i))) {
          datas.add(new XYChart.Data<>(eventNames.get(i) + " (" + repetitions + ")",
              soldPerEvent.get(i)));
          repetitions++;
          b = true;
          break;
