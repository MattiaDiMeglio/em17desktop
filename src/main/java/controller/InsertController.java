...
public class InsertController {
    ...
    public String getMaxVisitors(String location) {
        int i = 0;
        while (i < locationListModel.getLocationList().size()) {
            LocationModel locationModel = locationListModel.getLocationList().get(i);
            String locationName = locationModel.getLocationName() + "-" + locationModel.getLocationAddress();
            if (locationName.equals(location)) {
                //String maxVisitors;
                int j = 0;
                for (String s : locationModel.getSeatsList()) {
    ...
    /**
     * getter per {@link #imagesList}
     * @return {@link #imagesList}
     */
    ...
    /**
     * il metodo verifica se l'immagine che si sta eliminando è locale o meno.
     * se non è locale, viene eliminata dallo storage sul cloud
     * @param link link all'immagine
     */
    public void verifyAndDeleteImage(String link) {
        if (link.contains("http://") || link.contains("https://")){
            new StorageController().deleteFile(link);
        }
    }
}
