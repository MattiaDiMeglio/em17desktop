...
/**
 * MergedController si occupa di unire i dati di vari eventi così da inviarli al model una volta.
 * uniti
 *
 * @author ingSW20
 */
public class MergedController {
  ...
  private void merge() {
    ...
    for (EventModel eventModel : eventModelList) {
      ...
      mergedModel.setTicketSoldArray(
          mergeArray(eventModel.getTicketsSoldPerMonth(), mergedModel.getTicketsSoldArray()));

      //dati per il piechart
      mergedModel.setMaxVisitors(mergeMaxVisitors(mergedModel.getMaxVisitors(),
          Double.valueOf(eventModel.getMaxVisitors())));
      mergedModel.setTicketsSold(mergeTicketsSold(mergedModel.getTicketsSold(),
          Double.valueOf(eventModel.getTicketSold())));
    }
  ...
  /**
   * il metodo unisce i dati dei due paramentri in ingresso sommandoli e ne restituisce il
   * risultato
   *
   ...
   * @param maxVisitors1 numero massimo di visitatori da aggiungere al quello in {@link
   * #mergedModel}
   * @return somma dei due parametri
   */
   ...
   * risultato
   *
   * @param firstArray numero biglietti venduti divisi per mese (array con 12 elementi) già
   * contenuto in {@link #mergedModel}
   * @param secondArray numero biglietti venduti divisi per mese (array con 12 elementi) da
   * aggiungere in {@link #mergedModel}
   * @return somma dei due parametri
   */
  ...
  /**
   * aggiunge alla lista {@link #eventModelList} un nuovo evento e ne unisce i dati con quelli già
   * presenti
   *
